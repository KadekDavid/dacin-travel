    ...
    <main className="overflow-x-hidden">
      <section className="mx-auto max-w-7xl px-4 pb-12 pt-8 sm:px-6 sm:pb-16 sm:pt-12 lg:px-8 lg:pt-16">
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
          ...
          <div className="relative min-h-[280px] overflow-hidden rounded-lg bg-slate-50 shadow-sm sm:min-h-[420px]">
            <Image
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-3 lg:mt-14">
          {[
            ...
            <div key={label} className="ui-card p-5 text-center">
              <p className="m-0 text-3xl font-black text-blue-700">{value}</p>
              <p className="m-0 mt-2 text-sm font-semibold text-slate-600">{label}</p>
            </div>
          ))}
        </div>

        <section className="ui-card mt-10 p-5 sm:p-8 lg:mt-14">
          <div className="mb-6 max-w-2xl">
            <span className="text-xs font-bold uppercase tracking-wide text-blue-700">What guides us</span>
            <h2 className="m-0 mt-2 text-2xl font-extrabold text-slate-950 sm:text-3xl">Travel values that shape each package</h2>
          </div>
          <div className="grid gap-4 md:grid-cols-3">
            {values.map((value) => (
              <div key={value.title} className="rounded-lg border border-slate-200 bg-slate-50 p-5">
                <h3 className="m-0 text-lg font-bold text-slate-950">{value.title}</h3>
        ...
        <section className="mt-10 grid gap-6 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
          <div>
            <span className="text-xs font-bold uppercase tracking-wide text-blue-700">How it works</span>
            <h2 className="m-0 mt-2 text-2xl font-extrabold text-slate-950 sm:text-3xl">Simple planning from first idea to travel day</h2>
          </div>
          <div className="space-y-3">
            {steps.map((step, index) => (
              <div key={step} className="ui-card flex gap-4 p-4">
                <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md bg-blue-700 text-sm font-bold text-white">
                  {index + 1}
                </span>
                <p className="m-0 text-sm font-semibold leading-7 text-slate-700 sm:text-base">{step}</p>
              </div>
            ))}
          </div>
        </section>
